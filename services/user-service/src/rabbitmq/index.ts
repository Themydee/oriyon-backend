...
export async function connectRabbitMQ(url: string) {
  ...
  while (retries > 0) {
    try {
      ...
      connection.on("close", () => {
        console.error("[user-service][RabbitMQ] Connection closed. Reconnecting...");
        channel = null;
      ...
      retries--;
      console.error(`[user-service][RabbitMQ] Retrying... (${retries} left)`);
...
export async function publishEvent(routingKey: string, payload: Record<string, unknown>) {
  ...
  console.log(`[user-service][RabbitMQ] Published → ${routingKey}`);
}
...
export async function consumeEvent(
  ...
  channel.consume(queueName, async (msg) => {
    ...
    try {
      ...
      console.error(`[user-service][RabbitMQ] Handler error on ${routingKey}:`, err);
      channel!.nack(msg, false, false);
    }
  });
  console.log(`[user-service][RabbitMQ] Consuming → ${routingKey} (queue: ${queueName})`);
}
